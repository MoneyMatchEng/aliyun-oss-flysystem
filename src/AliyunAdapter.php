...
class AliyunAdapter implements FilesystemAdapter
{
    ...
    {
        try {
            $this->client->multiuploadFile($this->bucket, $this->prefixer->prefixPath($path), stream_get_meta_data($contents)["uri"], $this->options->mergeConfig($config, $this->visibility));
        } catch (OssException $exception) {
            throw UnableToWriteFile::atLocation($path, $exception->getMessage(), $exception);
        }
    }
